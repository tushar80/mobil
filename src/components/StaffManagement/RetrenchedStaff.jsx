...
function RetrenchedStaff(props) {
  ...
  useEffect(() => {
    setLoading(true)
    // axios.get(`${apiServerPrefix}/getRetrenchedStaff`)
    axios.get('https://randomuser.me/api/?results=5&inc=name,login')
      .then(res => {
        setRetrenchedStaff(res.data.results)
        setLoading(false)
      })
  ...
  const handleRecall = (e) => {
    ...
    // axios.get(apiServerPrefix + '/recallRetrench/' + id)
    //   .then(res => {
    //     if (res.status === 200) {
    setSnack({
      open: true,
    ...
    setRetrenchedStaff(retrenchedStaff.filter(s => s.login.uuid !== id))
